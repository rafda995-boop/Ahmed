...
Sound.SoundId = "rbxassetid://119437864395329";
Sound:Play();
...
ShowNoti("ZERO", "حسابي ديسكورد 2aqj", 103118967743041 )

task.delay(10, function()
    ShowNoti("ZERO", "حسابي انستا dragonlnx", 103118967743041 )
end)

local redzlib = loadstring(game:HttpGet("https://pastebin.com/raw/Ft4Fn6Hx"))()
...
local execSpeed = 100 -- السرعة الافتراضية
local hue = 0
local rgbConnection = nil

-- دالة نظام الـ RGB المطور
local function startHyperRGB()
    if rgbConnection then rgbConnection:Disconnect() end
    
    rgbConnection = RunService.RenderStepped:Connect(function(deltaTime)
        ...
        -- حساب اللون بناءً على السرعة المختارة
        -- نقسم execSpeed على 1000 للحصول على حركة ناعمة
        hue = (hue + (execSpeed / 10000)) % 1 
        local color = Color3.fromHSV(hue, 1, 1)
        
        -- إرسال اللون للسيرفر (نظام بروكهافن)
        pcall(function()
            local RE = ReplicatedStorage:FindFirstChild("RE")
            if RE and RE:FindFirstChild("1RPNam1eColo1r") then
                local event = RE["1RPNam1eColo1r"]
